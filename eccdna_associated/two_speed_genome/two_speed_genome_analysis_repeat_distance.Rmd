---
title: "two_speed_genome_analysis_repeat_distance"
...
reference_file = "moryzae_all_tes_wrepeats.bed"
...
closest_thing = function(x, output) {
  ...
  return(min(vector[is.finite(vector)]))
}
```
...
p1 <- ggplot(df, aes(x=distance_five, y=distance_three)) +  
  ...
  geom_segment(aes(x=x_int,xend=x_int,y=-Inf,yend=y_int), size =0.5, linetype='dashed') +
  geom_segment(aes(x=-Inf,xend=x_int,y=y_int,yend=y_int), size =0.5, linetype='dashed') +
  xlab("5' Distance to nearest repeat (kbp)") + ylab("3' Distance to nearest repeat (kbp)") +
  annotation_logticks() +
  scale_x_continuous(breaks=0:5, labels=c("0.01", "0.01", "0.1", "1", "10", "100"), limits = c(0.5,4.5)) +
  scale_y_continuous(breaks=0:5, labels=c("0.01", "0.01", "0.1", "1", "10", "100"), limits = c(0.5,4.5)) +
...
p2 <- ggplot(df[df$effector == TRUE,], aes(x=distance_five, y=distance_three)) +  
  ...
  geom_segment(aes(x=x_int,xend=x_int,y=-Inf,yend=y_int), size =0.5, linetype='dashed') +
  geom_segment(aes(x=-Inf,xend=x_int,y=y_int,yend=y_int), size =0.5, linetype='dashed') +
  geom_text(data = known_effectors_table, aes(x=distance_five, y=distance_three, label=known_effector_name)) +
  xlab("5' Distance to nearest repeat (kbp)") + ylab("3' Distance to nearest repeat (kbp)") +
  annotation_logticks() +
  scale_x_continuous(breaks=0:5, labels=c("0.01", "0.01", "0.1", "1", "10", "100"), limits = c(0.5,4.5)) +
  scale_y_continuous(breaks=0:5, labels=c("0.01", "0.01", "0.1", "1", "10", "100"), limits = c(0.5,4.5)) +
...
ggsave("two_speed_genome_analysis_repeat_distance_all_genes.pdf", plot = p1x, width = 3.25, height = 3.25)

ggsave("two_speed_genome_analysis_repeat_distance_effectors.pdf", plot = p2x, width = 3.25, height = 3.25)

ggsave("two_speed_genome_analysis_repeat_distance_never_found_genes.pdf", plot = p3x, width = 3.25, height = 3.25)

ggsave("two_speed_genome_analysis_repeat_distance_commonly_found_genes.pdf", plot = p4x, width = 3.25, height = 3.25)

```


```{r}
print('for whatever reason this next plot is absolutely required for knitting the html file but please disregard')
```
...
permutation = replicate(100, {
  sample_small <- df[sample(nrow(df), nrow(df[df$common_gene == "Never on eccDNAs",]) , replace = FALSE), ]
  sample_large <- df[! rownames(df) %in% rownames(sample_small), ]
  median(sample_small$min_distance)-median(sample_large$min_distance)
})
...
permutation = replicate(10000, {
  sample_small <- df[sample(nrow(df), nrow(df[df$common_gene == "Commonly on eccDNAs",]) , replace = FALSE), ]
  sample_large <- df[! rownames(df) %in% rownames(sample_small), ]
  median(sample_small$min_distance)-median(sample_large$min_distance)
})
...
p <- ggplot() + aes(permutation)+ geom_histogram(binwidth=10, colour="black", fill="white") + xlim(-200,200) + geom_vline(xintercept=observed)
